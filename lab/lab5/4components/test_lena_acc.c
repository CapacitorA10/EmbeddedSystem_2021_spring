        ...
        for(j = 1; j < 254; j+=2){
            ...
            for(k = 0; k <100; k++){
                ...
                *(sobel_accelerator+3) = row3;
                output = *(sobel_accelerator+4);
                out_img_v[i-1][j-1] = (unsigned char)(output >> 24);
                out_img_v[i-1][j] = (unsigned char)(output >> 16);
                out_img_v[i][j-1] = (unsigned char)(output >> 8);
                out_img_v[i][j] = (unsigned char)(output);
            }
